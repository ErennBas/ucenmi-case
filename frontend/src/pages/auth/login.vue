          ...
          <form @submit.prevent.stop="onSubmit">
            <q-card-section>
              <div class="q-gutter-md">
                ...
                <q-input ref="passwordRef" v-model="password" :rules="passwordRules" dense filled type="password"
                  label="Password" />
              </div>
            </q-card-section>
            <q-card-actions class="q-px-md">
              <q-btn :loading="loading" type="submit" dense unelevated color="light-green-7" size="lg" class="full-width"
                label="Login" />
            </q-card-actions>
            <q-card-section class="text-center q-pt-md">
              <q-btn flat class="text-grey-6" to="/auth/register">Created an Account</q-btn>
            </q-card-section>
          </form>
        </q-card>
      </div>
...
function onSubmit() {
  ...
  if (emailRef.value.validate() && passwordRef.value.validate()) {
    authService.login(email.value, password.value).then(res => {
      if (res.status === 200) {
